...
import { CURRENCY, VALIDATION } from '../config/constants';
import { useAuth } from '../contexts/AuthContext';
import { TimePicker } from '../shared/ui/inputs';
import { formatLocationForDisplay, getCurrentDeviceLocation, validateLocation } from '../utils/locationUtils';
...
const EnhancedCaregiverProfileWizard = ({ navigation, route }) => {
  ...
  // Background check request (calls Supabase RPC)
  const requestBackgroundCheck = async () => {
    try {
      if (!user?.id) {
        Alert.alert('Not signed in', 'You must be signed in to request a background check.');
        return;
      }

      setLoading(true);

      // Call Supabase RPC to create or update caregiver_background_checks and summary fields
      const { data, error } = await supabase.rpc('request_caregiver_background_check', {
        p_check_types: ['criminal_history', 'identity', 'employment_history', 'references'],
      });

      if (error) {
        console.error('Background check RPC failed:', error);
        Alert.alert(
          'Request Failed',
          error.message || 'Failed to submit background check request. Please try again.'
        );
        return;
      }
